...
2. **实现类 internal 化**（public 面收缩）：`ToolCatalog`/`ToolRegistry`/`NavigationService`/`ViewMappingEntry`（Module.Base）+ `ThemeService`/`ClipboardService`/`FolderPickerService`（Core），Module.Base 加 `InternalsVisibleTo("DS.Tools.Tests")`（测试直接构造 internal 实现）；`SubToolInfo.cs` 移项目根（命名空间不变，零 XAML/using 改动）；空目录清理（Tempds-tools-build ×7 等）
